...
OBFUSCATE_URL = 'https://joint-marten-virtually.ngrok-free.app/obfuscate'
PASTE_BASE_URL = 'https://joint-marten-virtually.ngrok-free.app/paste'
...
class MethodPanel(View):
    ...
    @discord.ui.button(label='Raw', style=discord.ButtonStyle.success)
                            ...
                            if resp.status == 200:
                                ...
                                if paste_id:
                                    if method_selected == 'raw':
                                        paste_url = f'{PASTE_BASE_URL}/{paste_id}/raw'
                                        await message.channel.send(f'Your obfuscated code is available at: {paste_url}')
                                    elif method_selected == 'download':
                                        paste_url = f'{PASTE_BASE_URL}/{paste_id}'
                                        async with session.get(paste_url) as paste_resp:
                                            if paste_resp.status == 200:
                                                paste_data = await paste_resp.json()
                                                obfuscated_code = paste_data['content']
                                                file = discord.File(fp=io.StringIO(obfuscated_code), filename=f'{paste_id}.lua')
                                ...
                                await message.channel.send('Error: Failed to obfuscate the code.')
